...
プロジェクト(git root)の `.agents/skills-config/retro/config.json` を読む。`runs` 配列の最後の `timestamp` が前回実行時刻で、これが分析範囲の起点になる。ファイルが無い・`runs` が空なら初回実行として直近 7 日を起点にする。
...
適用完了後、上記 config.json(無ければ作成)の `runs` 末尾にエントリを追記する。**適用ゼロでも必ず記録する** (次回実行の分析起点になるため)。現在時刻は `date -Iseconds` で取得する。
