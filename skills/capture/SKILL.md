...
プロジェクト(git root)の `.agents/skills-config/capture/config.json` を読み込み、過去にハマったポイント(起動待ち・認証・描画タイミングなど)を把握して撮影時に回避する。ファイルが無ければスキップして続行する。
...
撮影中に発生した問題(起動待ちの不足、認証の再要求、描画前の撮影など)があれば、上記 config.json(無ければ作成)に `issue` / `cause` / `workaround` / `date` の形式で記録する。
