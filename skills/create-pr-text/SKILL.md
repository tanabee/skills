...
**pr.md 本文に `tmp/` 配下のファイルへの参照(パス・リンク)を書かない。** `tmp/` は .gitignore 対象でレビュアは到達できない。plan.md や implementation-notes 等の内容が必要な場合はパスを示すのではなく要点を本文に転記する。例外は「UI の変更」テーブルの img src のみ(GitHub 貼り付け時に差し替える前提のプレースホルダ)。
