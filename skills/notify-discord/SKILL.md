...
1. `~/.agents/.skills-config/notify-discord/config.json` の `projects.<プロジェクト名>`（プロジェクト名は git root のディレクトリ名）から `webhook_url` を取得する。存在しない場合はユーザーに Webhook URL を質問し、同エントリに保存する（ファイルやディレクトリが無ければ作成する）
